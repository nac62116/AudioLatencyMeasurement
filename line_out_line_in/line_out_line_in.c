 ...
 * --- Pros: Very Fast (TOTAL_MEASUREMENTS / SIGNAL_FREQUENCY_IN_HZ + latency of the DUT in seconds; Unit = seconds)
 * --- Cons: Invalid measurements by losing/adding a signal through random noise
 * 
 * v1.2: Iterative measurement, condition = measurement frequency > latency of the DUT
 * --- Pros: No invalidation of measurements by losing/adding a signal through random noise
 * --- Cons: Rather slow (TOTAL_MEASUREMENTS * MEASUREMENT_FREQUENCY_IN_S, where MEASUREMENT_FREQUENCY_IN_SECONDS > latency of the DUT in seconds; Unit = seconds)
 * --------: To know the right value for MEASUREMENT_FREQUENCY_IN_S one got to know/estimate the value of the latency of the DUT
 * 
 * v1.2.1: Iterative measurement, condition = measurement frequency > latency of DUT only for calibration measurements
 * --- Pros: No invalidation of measurements by losing/adding a signal through random noise
 * --------: No need to know/estimate the latency of the DUT
 * --------: Reducing time in comparance to v1.2 by starting with a high MEASUREMENT_FREQUENCY_IN_S (above all DUT latencies)
 *           and then reducing it after TOTAL_CALIBRATION_MEASUREMENTS to maxLatency + MEASUREMENT_FREQUENCY_MARGIN
 * --- Cons: Still slower than v1.1
 * 
 ...
 * --- Pros: Faster than v1.2 and v1.2.1
 * --------: No need to know/estimate the latency of the DUT
 * --- Cons: Invalid measurements by losing/adding a signal through random noise
 * --------: Slower than v1.1
 */
